...
def log_event(ip, action, status, user_agent):
    ...
    with sqlite3.connect(DB_PATH) as conn:
        conn.execute("INSERT INTO logs (time, ip, location, device, action, status) VALUES (?, ?, ?, ?, ?, ?)",
...
def text_engine():
    ...
    try:
        ...
        if mode == 'encrypt':
            ...
            tag = encrypted_data[2+len_dek+12 : 2+len_dek+12+16]
            ciphertext = encrypted_data[2+len_dek+12+16:]
            ...
            try:
                ...
                return _corsify_response(jsonify({"error": "Access Denied: Invalid Password"}), 403)
...
def file_engine():
    ...
    try:
        ...
        if not file or not password:
            return _corsify_response(jsonify({"error": "Missing file or key"}), 400)

        file_bytes = file.read()
        filename = file.filename

        if mode == 'encrypt':
            # Handle Image Conversion if requested
            if format_choice != 'original':
                try:
                    ...
                    img.save(out_buffer, format=format_choice.upper())
                    file_bytes = out_buffer.getvalue()
                    filename = filename.rsplit('.', 1)[0] + f".{format_choice}"
                except Exception: pass # If not an image, just ignore format choice
            ...
            tag = file_bytes[2+len_dek+12 : 2+len_dek+12+16]
            
            len_name = int.from_bytes(file_bytes[2+len_dek+12+16 : 2+len_dek+12+16+1], 'big')
            orig_name = file_bytes[2+len_dek+12+16+1 : 2+len_dek+12+16+1+len_name].decode('utf-8')
            ciphertext = file_bytes[2+len_dek+12+16+1+len_name : ]
...
def panic_wipe():
    ...
    return _corsify_response(jsonify({"status": "Logs Wiped"}))
